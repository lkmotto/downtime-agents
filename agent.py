    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code in (200, 201, 204):
                logger.info(f"Backend: pushed {len(events)} events for {city.name} → {resp.status_code}")
                return True
            else:
                logger.warning(
...
async def _process_city(city: CityConfig, dry_run: bool, run_ts: str) -> dict[str, Any]:
    ...
    # ── AllEvents.in ──
    try:
        ae_events = await fetch_allevents_events(city.name, city.state, city_slug=city.slug)
